...
#March 2021
#    parser.add_argument('-bbox', dest='bbox', type=float, default=None,
#            help = 'user input bounding box, format: s/n/w/e. default: bbox of ref_date_stack_track')
    parser.add_argument('-bbox', '--bbox', type = float, default = None, nargs = '+', dest = 'bbox', help = 'Defines the spatial region in the format south north west east.\
                        The values should be float from (-90,90) for latitudes and (0,360) or (-180,180) for longitudes.')
...
if __name__ == '__main__':
    ...
    if bbox is not None:
        #bbox = [float(x) for x in bbox.split('/')] #March, 2021
