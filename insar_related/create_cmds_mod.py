...
'''.format(array)
    ...
    #start new commands: estimate SLC offsets, .sh file
    #################################################################################
    if datesProcessSecondary != []:
        extraArguments = ''
        ...
        cmd  = '#!/bin/bash\n\n'
        cmd += header('estimate SLC_offsets')
        cmd += '\n'
        cmd += '#########################################################################\n'
   ...
   qsub -v sec_date=${dates[i]} pbs_1b.pbs
        ...
        cmd += '\n'
        cmd += '# load isce\n'
        cmd += 'source /home/share/insarscripts/isce2-code/isce2_alos2stack/isce_env.sh\n'
        cmd += '\n'
        cmd += '# set option\n'
        ...
        cmd += '   dem={}\n'.format(stack.dem)
        cmd += '\n'
        cmd += '   $code -idir dates -ref_date $ref_date -sec_date $sec_date -wbd $wbd -dem $dem {}\n'.format(extraArguments)
        cmd += '\n'
        cmd += 'fi\n'
    ...
    #start new commands: estimate swath and frame offsets
    #################################################################################
    ...
    #start new commands: resample to a common grid, .sh file
    #################################################################################
    if datesProcess != []:
        extraArguments = ''
   ...
   qsub -v sec_date=${dates[i]} pbs_1d.pbs
        ...
        cmd += '\n'
        cmd += '   $code -idir dates -odir dates_resampled -ref_date $ref_date -sec_date $sec_date -nrlks1 $numberRangeLooks1 -nalks1 $numberAzimuthLooks1 {}\n'.format(extraArguments)
        cmd += '\n'
        cmd += 'fi\n'
    ...
    #start new commands: mosaic parameter and compute lon/lat/hgt
    #################################################################################
    if datesProcess != []:
        cmd  = '#!/bin/bash\n'
        cmd += '\n'
    ...
    #start new commands: compute geometrical offsets, .sh file
    if datesProcessSecondary != []:
        extraArguments = ''
        if insar.useGPU:
            extraArguments += ' -gpu'
   ...
   qsub -v sec_date=${dates[i]} pbs_1f.pbs
        ...
        cmd += '   numberAzimuthLooks1={}\n'.format(insar.numberAzimuthLooks1)
        cmd += '\n'
        cmd += '   $code -date $sec_date -date_par_dir ../../dates/$sec_date -lat $lat -lon $lon -hgt $hgt -nrlks1 $numberRangeLooks1 -nalks1 $numberAzimuthLooks1 {}\n'.format(extraArguments)
        cmd += '   cd ../../'
        cmd += '\n\n'
        cmd += 'fi\n'
    ...
    #start new commands: form and mosaic interferograms, .sh file
    if pairsProcess != []:
        #################################################################################
        cmd  = '#!/bin/bash\n\n'
   ...
   qsub -v pair=${insarpair[i]} pbs_2b.pbs
    ...
    #start new commands: estimate residual offsets between radar and DEM
    if processDateReferenceStack:
    #if not os.path.isfile(os.path.join(stack.datesResampledDir, stack.dateReferenceStack, 'insar', 'affine_transform.txt')):
    ...
    #start new commands: rectify range offsets, .sh file
    if datesProcessSecondary != []:
        cmd  = '#!/bin/bash\n\n'
   ...
   qsub -v date=${dates[i]} pbs_2d.pbs
   ...
   qsub -v pair=${insarpair[i]} pbs_2e.pbs
    ...
    #start new commands: ionospheric phase estimation, .sh file
    if insar.doIon and (pairsProcessIon != []):
        cmd  = '#!/bin/bash\n\n'
   ...
   qsub -v ionpair=${ionpair[i]} pbs_3b.pbs
...
for ((i=0;i<${{#insarpair[@]}};i++)); do
   ...
   qsub -v insarpair=${insarpair[i]} pbs_4a.pbs
        ...
        cmd += "   unw=$insarpair'_'$nrlks'rlks_'$nalks'alks.unw'\n"
        cmd += "   mskUnw=$insarpair'_'$nrlks'rlks_'$nalks'alks_msk.unw'\n"
        cmd += "   los=$insarpair'_'$nrlks'rlks_$nalks'nalks.los\n"
        cmd += '\n'
        cmd += '   cd insar\n'
        cmd += '   $code -ref_date_stack_track $xmlFile -dem $demGeo -input $cor -nrlks $nrlks -nalks $nalks\n'
        cmd += '   $code -ref_date_stack_track $xmlFile -dem $demGeo -input $unw -nrlks $nrlks -nalks $nalks\n'
        cmd += '   $code -ref_date_stack_track $xmlFile -dem $demGeo -input $mskUnw -nrlks $nrlks -nalks $nalks\n'
        cmd += '   cd ../../../\n'
        cmd += '\n'
        cmd += '   cd {}/{}/insar\n'.format(stack.datesResampledDir,stack.dateReferenceStack)
        cmd += '   $code -ref_date_stack_track $xmlFile -dem $demGo -input $los -nrlks $nrlks -nalks $nalks\n'
        cmd += '   cd ../../../\n'
        cmd += '\n'
    ...
    cmd += 'source /home/share/insarscripts/isce2-code/isce2_alos2stack/isce_env.sh\n'
    cmd += '\n'
    cmd += '#read data, compute baseline and burst synchronization\n'
    cmd += '#./cmd_1a.sh\n'
    cmd += '\n'
    cmd += '#estimate SLC offset\n'
    cmd += '#./cmd_1b.sh <-- run it directly from the headnode\n'
    cmd += '\n'
    cmd += '#estimate swath and frame offsets\n'
    cmd += '#./cmd_1c.sh\n'
    cmd += '\n'
    cmd += '#resample to a common grid\n'
    cmd += '#./cmd_1d.sh <-- run it directly from the headnode\n'
    cmd += '\n'
    cmd += '#mosaic parameter, compute latitude, longitude and height\n'
    cmd += '#./cmd_1e.sh\n'
    cmd += '\n'
    cmd += '#compute geometrical offsets\n'
    cmd += '#./cmd_1f.sh <-- run it directly from the headnode\n'
    cmd += '\n'
    cmd += '#pair up scenes\n'
    cmd += '#./cmd_2a.sh\n'
    cmd += '\n'
    cmd += '#form and mosaic interferograms\n'
    cmd += '#./cmd_2b.sh <-- run it directly from the headnode\n'
    cmd += '\n'
    cmd += '#estimate residual offsets between radar and DEM\n'
    cmd += '#./cmd_2c.sh\n'
    cmd += '\n'
    cmd += '#rectify range offsets\n'
    cmd += '#./cmd_2d.sh <-- run it directly from the headnode\n'
    cmd += '\n'
    cmd += '#diff interferograms, look and coherence, then do look geom\n'
    cmd += '#./cmd_2e.sh <-- run it directly from the headnode\n'
    cmd += '\n'
    cmd += '#pair up for ionospheric phase estimation\n'
    cmd += '#./cmd_3a.sh\n'
    cmd += '\n'
    cmd += '#ionospheric phase estimation\n'
    cmd += '#./cmd_3b.sh <-- run it directly from the headnode\n'
    cmd += '\n'
    cmd += '#prepare interferograms for/and checking ionospheric estimation results\n'
    cmd += '#./cmd_3c.sh\n'
    cmd += '\n'
    cmd += '#estimate ionopheric phase for each date, then correct ionophere\n'
    cmd += '#before running this, check the instructions inside\n'
    cmd += '#./cmd_3d.sh\n'
    cmd += '\n'
    cmd += '#filter, unwrap and geocode interferograms\n'
    cmd += '#./cmd_4a.sh <-- run it directly from the headnode\n'

    #save commands
    pbs4b = cmd


    return (cmd1a, cmd1b, pbs1b, cmd1c, cmd1d, pbs1d, cmd1e, cmd1f, pbs1f, cmd2a, cmd2b, pbs2b, cmd2c, cmd2d, pbs2d, cmd2e, pbs2e, cmd3a, cmd3b, pbs3b, cmd3c, cmd3d, cmd4a, pbs4a, pbs4b)
...
if __name__ == '__main__':
    ...
    if (datesProcess == []) and (pairsProcess == []) and (pairsProcessIon == []):
        ...
        cmd1a, cmd1b, pbs1b, cmd1c, cmd1d, pbs1d, cmd1e, cmd1f, pbs1f, cmd2a, cmd2b, pbs2b, cmd2c, cmd2d, pbs2d, cmd2e, pbs2e, cmd3a, cmd3b, pbs3b, cmd3c, cmd3d, cmd4a, pbs4a, pbs4b = createCmds(stack, datesProcess, pairsProcess, pairsProcessIon, mode)
        ...
        with open('cmd_1c.sh', 'w') as f:
            f.write(cmd1c)
        with open('cmd_1d.sh', 'w') as f:
            f.write(cmd1d)
        with open('pbs_1d.pbs', 'w') as f:
            f.write(pbs1d)
        with open('cmd_1e.sh', 'w') as f:
            f.write(cmd1e)
        with open('cmd_1f.sh', 'w') as f:
            f.write(cmd1f)
        with open('pbs_1f.pbs', 'w') as f:
            f.write(pbs1f)
        ...
        with open('cmd_2c.sh', 'w') as f:
            f.write(cmd2c)
        with open('cmd_2d.sh', 'w') as f:
            f.write(cmd2d)
        with open('pbs_2d.pbs', 'w') as f:
            f.write(pbs2d)
        with open('cmd_2e.sh', 'w') as f:
            f.write(cmd2e)
        with open('pbs_2e.pbs', 'w') as f:
            f.write(pbs2e)
        with open('cmd_3a.sh', 'w') as f:
            f.write(cmd3a)
        with open('cmd_3b.sh', 'w') as f:
            f.write(cmd3b)
        with open('pbs_3b.pbs', 'w') as f:
            f.write(pbs3b)
        with open('cmd_3c.sh', 'w') as f:
            f.write(cmd3c)
        with open('cmd_3d.sh', 'w') as f:
            f.write(cmd3d)
        with open('cmd_4a.sh', 'w') as f:
            f.write(cmd4a)
        with open('pbs_4a.pbs', 'w') as f:
            f.write(pbs4a)
        with open('run_alos2Stack_cmds.pbs', 'w') as f:
            f.write(pbs4b)

    runCmd('chmod +x cmd_1a.sh cmd_1b.sh cmd_1c.sh cmd_1d.sh cmd_1e.sh cmd_1f.sh cmd_2a.sh cmd_2b.sh cmd_2c.sh cmd_2d.sh cmd_2e.sh cmd_3a.sh cmd_3b.sh cmd_3c.sh cmd_3d.sh cmd_4a.sh', silent=1)
