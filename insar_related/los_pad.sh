...
if [[ $# -lt 1 ]]; then
  ...
  if [[ "$1" == "-h" ]]; then
    echo "usage: $0 [grdfilename] [-Rbounds]"
    echo "computes the exterior bounds matching all files intf/*/[name] and pads them all to match"
    echo "default: unwrap_mask_ll.grd"
    echo "if a second argument (bounds) is given, also do not write out the corresponding intf.in file"
    exit 0
  fi
  fname=$1
fi
...
if [[ $# -lt 2 ]]; then
  echo "renaming old intf.in"
  mv intf.in intf.in.bak
fi
