...
def getLengthWdithPixelSize(unwgeo):
    
    metadata = {}

    # length and width
    ds = gdal.Open(unwgeo, gdal.GA_ReadOnly)
...
if __name__ == '__main__':
	...
	# read file length, width, pixel size, and upper left corner coordinates
        ...
        metadata = getLengthWdithPixelSize(unwgeo)
	...
	# write to .rsc file
        for i in range(3):
            if i==0:
               ...
               metadata['UNIT'] = '1' 
               corgeo = glob.glob('insar/*.cor.geo')[0]
               rsc_file = corgeo + '.rsc'
               metadata = readfile.standardize_metadata(metadata)
               if rsc_file:
                  print('writing ', rsc_file)
    ...
    # prepare dem and its metadata
    unwgeo = glob.glob(os.path.join(pairs[0], 'insar', 'filt_*lks.unw.geo'))[0]
    getDem(unwgeo)
